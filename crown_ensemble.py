...
import keras
from keras.layers import BatchNormalization, Conv2D, Dense, Flatten, MaxPooling2D, Concatenate, SeparableConv2D
...
class Ensemble(object):
    ...
    def __init__(self, settings_file) -> None:
        ...
        self.settings = self._clean_and_check(self.settings)
        self.data = _load_data(self.settings)

        self.model = _initialize_model()

        
    def _clean_and_check(sf):
        a = None
    ...
    def _initialize_model():
        ...
        rf_strings = ['rf','random_forest','random forest']
        rf_strings = ['nn','neural_network','neural network']
        
        if (sf['model'].lower() in rf_strings):
            model = RandomForestRegressor(n_estimators=200, max_depth=2, random_state=13)
        if (sf['model'].lower() in nn_strings):

            # TODO: fix X.shape after you've handled data import and can pull from there.
            inlayer = keras.layers.Input(shape=(X.shape[1],))
            output_layer = inlayer
            output_layer = Dense(units=20, activation='relu')(output_layer)
            ...
            model.fit(X,Y,validation_data=(val_X,val_Y),epochs=29)

        else:
            print('Invalid model name {}'.format(sf['model']))
            quit()
    ...
    def apply_model()
        ...
        if (self.settings['scale response']):
          pred = y_scaler.inverse_transform(pred)

        pred = apply_transform(pred,self.settings['chem transforms'][_chem],invert=True)
...
def simple_nn(X,Y,val_X,val_Y):
    ...
    score = np.sqrt(np.mean(np.power(np.squeeze(pred_Y)-np.squeeze(val_Y),2)))

    return model, score


def simple_rf(X,Y,val_X,val_Y):
    model.fit(X,Y)
    pred_Y = model.predict(val_X)
...
for _chem in range(0,len(chemlist)):
  ...
  for iter in range(0,sf.get_setting('iterations')):
    ...
    if (np.sum(validation) > 0):
        model, score = simple_rf(X_train,Y_train,X_validation,Y_validation)
    else:
        model, score = simple_rf(X_train,Y_train,X_train,Y_train)
